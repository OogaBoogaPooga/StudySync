...
/* ---------- Chat with your notes ---------- */  const CHAT_STOPWORDS = new Set([   'the','a','an','and','or','but','of','to','in','on','at','for','with','by',   'is','are','was','were','be','been','being','do','does','did','what','who',   'whom','whose','which','when','where','why','how','that','this','these',   'those','it','its','as','from','into','about','can','could','should','would',   'will','shall','may','might','i','you','he','she','they','we','me','him',   'her','them','us','my','your','his','their','our','explain','describe','tell',   'give','list','name','define', ]);  function tokenize(str) {   return String(str || '')     .toLowerCase()     .replace(/[^a-z0-9 ]/g, ' ')     .split(/\s+/)     .filter((w) => w.length > 2 && !CHAT_STOPWORDS.has(w)); }  function rankCards(question, cards) {   const qTokens = tokenize(question);   if (!qTokens.length) return [];   const qSet = new Set(qTokens);   const qLower = question.toLowerCase();    const scored = cards.map((card) => {     const frontTokens = tokenize(card.front);     const backTokens = tokenize(card.back);     let score = 0;     frontTokens.forEach((t) => { if (qSet.has(t)) score += 3; });     backTokens.forEach((t) => { if (qSet.has(t)) score += 1; });     if (card.front && qLower.includes(card.front.toLowerCase())) score += 5;     return { card, score };   });    return scored     .filter((s) => s.score > 0)     .sort((a, b) => b.score - a.score)     .slice(0, 6)     .map((s) => s.card); }  router.post('/chat', validate(z.object({   message: z.string().trim().min(1),   setId: z.string().nullable().optional(), })), wrap(async (req, res) => {   const { message, setId } = req.body;    let cards = [];   let scopeLabel = 'all of your study sets';    if (setId) {     const set = await prisma.studySet.findFirst({       where: { id: setId, userId: req.user.id },       include: { cards: true },     });     if (!set) return res.status(404).json({ error: 'Set not found' });     cards = set.cards.map((c) => ({ ...c, setTitle: set.title }));     scopeLabel = `the study set "${set.title}"`;   } else {     const sets = await prisma.studySet.findMany({       where: { userId: req.user.id },       include: { cards: true },     });     cards = sets.flatMap((s) => s.cards.map((c) => ({ ...c, setTitle: s.title })));   }    if (!cards.length) {     return res.json({       reply: "You don't have any cards yet. Add some to a study set and I'll be able to help.",       sources: [],     });   }    const top = rankCards(message, cards);   const passages = top.length     ? top.map((c, i) => `[${i + 1}] ${c.front} — ${c.back} (from "${c.setTitle}")`).join('
')     : '(No strongly matching cards found.)';    const systemPrompt = `You are StudySync's study assistant. Answer the user's question using ONLY the information in the passages below, drawn from ${scopeLabel}. Rules: - If the answer is not present in the passages, reply exactly: "I couldn't find that in your notes." - Be concise: 1-3 short paragraphs max. - When you use a passage, cite it inline like [1], [2]. - Do not invent facts. Do not use outside knowledge.`;    if (!process.env.GROQ_API_KEY) {     return res.json({       reply: 'AI is not configured. Add GROQ_API_KEY to your environment.',       sources: [],     });   }    const reply = await callAI({     systemPrompt,     userPrompt: `Passages:
${passages}

Question: ${message}`,     maxTokens: 1200,   });    await prisma.chatMessage.createMany({     data: [       { userId: req.user.id, setId: setId || null, role: 'user', content: message },       { userId: req.user.id, setId: setId || null, role: 'assistant', content: reply },     ],   });    res.json({     reply,     sources: top.map((c) => ({ front: c.front, back: c.back, setTitle: c.setTitle })),   }); }));  export default router;;
